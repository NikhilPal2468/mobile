#!/usr/bin/env bash
# Build Android release APK locally. Requires JDK 17.
# Uses same keystore/setup as AAB build. Good for sideloading or sharing with testers.
...
# Force project-local Gradle cache
...
cd "$ANDROID_ROOT"
GRADLE_USER_HOME="$GRADLE_USER_HOME" ./gradlew --no-daemon assembleRelease

APK="$APP_ROOT/build/outputs/apk/release/app-release.apk"
echo "Done. APK: $APK"
